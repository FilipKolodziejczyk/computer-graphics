...
Cylinder::Cylinder(double radius, double height, int sides, Vector positionOnScene) : radius(radius), height(height) {
    this->vertices = QList<Vector>(4 * sides + 2);
    this->triangles = QList<TriangleVertices>(4 * sides);
    this->triangles.reserve(4 * sides);
    this->textureCoordinates = QList<Vector>(4 * sides + 2);

    // Top base
    this->vertices[0] = Vector(0, height, 0, 1); // Base center vertex
    for (int i = 0; i < sides; i++) {
        double angle = 2 * M_PI * i / sides;
        this->vertices[i + 1] = Vector(radius * cos(angle), height, radius * sin(angle), 1); // Side vase vertices
    }
    ...
    // Top base texture coords
    textureCoordinates[0] = Vector(0.25, 0.25);
    for (int i = 0; i < sides; i++) {
        textureCoordinates[i + 1] = Vector(0.25 * (1 + cos(2 * M_PI * i / sides)),
                                           0.25 * (1 + sin(2 * M_PI * i / sides)));
    }

    // Side
    for (int i = sides; i < 2 * sides; i++) {
        double angle = 2 * M_PI * (i - sides) / sides;
        this->vertices[i + 1] = Vector(radius * cos(angle), height, radius * sin(angle), 1);
    }
    ...
    for (int i = 2 * sides; i < 3 * sides; i++) {
        ...
        this->vertices[i + 1] = Vector(radius * cos(angle), 0, radius * sin(angle), 1);
    }
    ...
    // Side texture coords
    for (int i = 0; i < sides; i++) {
        textureCoordinates[sides + i + 1] = Vector(i * 1.0 / (sides - 1), 1);
        textureCoordinates[2 * sides + i + 1] = Vector(i * 1.0 / (sides - 1), 0.5);
    }
    ...
    for (int i = 3 * sides; i < 4 * sides; i++) {
        ...
        this->vertices[i + 1] = Vector(radius * cos(angle), 0, radius * sin(angle), 1); // Side base vertices
    }
    this->vertices[4 * sides + 1] = Vector(0, 0, 0, 1); // Base center vertex
...
// Bottom base texture coords
    textureCoordinates[4 * sides + 1] = Vector(0.75, 0.25);
    for (int i = 0; i < sides; i++) {
        textureCoordinates[3 * sides + i + 1] = Vector(0.25 * (3 + cos(2 * M_PI * i / sides)),
                                                       0.25 * (1 + sin(2 * M_PI * i / sides)));
    }

    this->transformation.dst = std::move(positionOnScene);
}